...
it('caps render pixels on a high resolution display without forcing upsampling',()=>{
 for(const [quality,budget] of [['quality',4_000_000],['auto',2_000_000],['performance',1_000_000]] as const){const ratio=budgetPixelRatio(1.85,7680,4320,quality);expect(7680*4320*ratio*ratio).toBeLessThanOrEqual(budget+1);}
 expect(budgetPixelRatio(.65,390,844,'auto')).toBe(.65);
});
