...
export async function createRuLabView(options:ViewOptions):Promise<RuLabView>{
  ...
  function applyMode(){const external=Boolean(imported||capture);entities.root.visible=!external;controls.setCollisionEnabled(!external);architecture.meshes.visible=!external&&mode!=='splats';architecture.splats.visible=!external&&mode!=='cinematic';architecture.splats.opacity=mode==='graph'?.16:.96;cinema.setAuthored(!external&&mode==='cinematic');entities.setGraph(!external&&mode==='graph');atmosphere.visible=!external&&!photographic?.hdri;scene.background=external?new T.Color(0x141c1e):photographic?.sky??null;scene.backgroundIntensity=.7;scene.backgroundBlurriness=.08;scene.fog=external||mode==='splats'?null:new T.FogExp2(0x777b7b,.014);}
  ...
  void loadPhotographicAssets(renderer,architecture.meshes,scene,()=>disposed).then(value=>{photographic=value;if(!disposed){applyMode();options.onMetrics(metrics(0,0));}});
